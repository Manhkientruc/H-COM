...
<?php
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT id_mh, hinhanh, ten_mh, giaban, hang, model, kichthuoctile, dophangiai, tansoquet, thongtin FROM tbl_mh WHERE id_mh = $id";
        ...
        if ($result->num_rows > 0) {
            ...
            echo '<h2>' . $row['ten_mh'] . '</h2>';
            echo '<h3></h3>';
            echo 'Giá : <h1 class="editchut">' . number_format($row["giaban"] - 1500000) . 'đ</h1><br>';
            echo '<h1>THÔNG SỐ KỸ THUẬT : </h1>';
            echo '<h3>Hãng:</h3>';
            echo '<p>' . $row['hang'] . '</p>';
            echo '<h3 class="light-grey">Model:</h3>';
            echo '<p class="light-grey">' . $row['model'] . '</p>';
            echo '<h3>Kích thước và tỉ lệ:</h3>';
            echo '<p>' . $row['kichthuoctile'] . '</p>';
            echo '<h3 class="light-grey">Độ phân giải:</h3>';
            echo '<p class="light-grey">' . $row['dophangiai'] . '</p>';
            echo '<h3>Tần số quét:</h3>';
            echo '<p>' . $row['tansoquet'] . '</p>';
            echo '</div>';
            echo '<div class="laptop-details">';
            echo '<div class="details-wrapper">';
            ...
            echo "Không tìm thấy thông tin chi tiết cho Màn hình này.";
        }
    } else {
        echo "Không tìm thấy ID của Màn hình.";
    }
