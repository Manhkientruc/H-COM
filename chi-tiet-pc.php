...
<?php
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT id_pc, hinhanh, ten_pc, giaban, cpu, main, vga, ram, SSD, tan, nguon, thongtin FROM tbl_pc WHERE id_pc = $id";
        ...
        if ($result->num_rows > 0) {
            ...
            echo '<h3>MAIN:</h3>';
            echo '<p>' . $row['main'] . '</p>';
            echo '<h3 class="light-grey">VGA:</h3>';
            echo '<p class="light-grey">' . $row['vga'] . '</p>';
            echo '<h3>RAM:</h3>';
            echo '<p>' . $row['ram'] . '</p>';
            echo '<h3 class="light-grey">SSD/HDD:</h3>';
            echo '<p class="light-grey">' . $row['SSD'] . '</p>';
            echo '<h3>Tản:</h3>';
            echo '<p>' . $row['tan'] . '</p>';
            echo '<h3 class="light-grey">Nguồn:</h3>';
            echo '<p class="light-grey">' . $row['nguon'] . '</p>';
            echo '</div>';
            echo '<div class="laptop-details">';
            echo '<div class="details-wrapper">';
            ...
            echo "Không tìm thấy thông tin chi tiết cho PC này.";
        }
    } else {
        echo "Không tìm thấy ID của PC.";
    }
