...
<div class="menu">
    ...
    $sql = "SELECT id_lk, hinhanh, ten_lk, giaban FROM tbl_lk";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        echo '<div class="grid-container">';
        while ($row = $result->fetch_assoc()) {
            echo '<a href="chi-tiet-lk.php?id=' . $row['id_lk'] . '" class="grid-item">';
            echo '<img src="data:image/jpeg;base64,' . base64_encode($row['hinhanh']) . '">';
            echo '<p>' . $row['ten_lk'] . '</p>';
            echo '<h4>' . number_format($row["giaban"]) . 'đ</h4>';
            echo '<h3></h3>';
            echo '<h1>' . number_format($row["giaban"] - 500000) . 'đ</h1>';
            }
        echo '</div>';
    } else {
...
<?php
    ...
    $sql = "SELECT id_gpu, hinhanh, ten_gpu, giaban FROM tbl_gpu";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        echo '<div class="grid-container">';
        while ($row = $result->fetch_assoc()) {
            echo '<a href="chi-tiet-gpu.php?id=' . $row['id_gpu'] . '" class="grid-item">';
            echo '<img src="data:image/jpeg;base64,' . base64_encode($row['hinhanh']) . '">';
            echo '<p>' . $row['ten_gpu'] . '</p>';
            echo '<h4>' . number_format($row["giaban"]) . 'đ</h4>';
            echo '<h3></h3>';
            echo '<h1>' . number_format($row["giaban"] - 500000) . 'đ</h1>';
            echo '</a>';
