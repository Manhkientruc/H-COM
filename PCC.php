...
<div class="menu">
    ...
    $sql = "SELECT id_pc, hinhanh, ten_pc, giaban FROM tbl_pc";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        echo '<div class="grid-container">';
        while ($row = $result->fetch_assoc()) {
            echo '<a href="chi-tiet-pc.php?id=' . $row['id_pc'] . '" class="grid-item">';
            echo '<img src="data:image/jpeg;base64,' . base64_encode($row['hinhanh']) . '">';
            echo '<p>' . $row['ten_pc'] . '</p>';
            echo '<h4>' . number_format($row["giaban"]) . 'đ</h4>';
            echo '<h3></h3>';
            echo '<h1>' . number_format($row["giaban"] - 1500000) . 'đ</h1>';
            }
        echo '</div>';
    } else {
