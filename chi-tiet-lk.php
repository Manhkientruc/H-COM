...
<?php
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT id_lk, hinhanh, ten_lk, giaban, hang, model, nhan, luong, dien, nhiet, thongtin FROM tbl_lk WHERE id_lk = $id";
        ...
        if ($result->num_rows > 0) {
            ...
            echo '<h3>Model:</h3>';
            echo '<p>' . $row['model'] . '</p>';
            echo '<h3 class="light-grey">Số nhân:</h3>';
            echo '<p class="light-grey">' . $row['nhan'] . '</p>';
            echo '<h3>Số luồng:</h3>';
            echo '<p>' . $row['luong'] . '</p>';
            echo '<h3 class="light-grey">Điện áp tiêu thụ:</h3>';
            echo '<p class="light-grey">' . $row['dien'] . '</p>';
            echo '<h3>Nhiệt độ tối đa:</h3>';
            echo '<p>' . $row['nhiet'] . '</p>';
            echo '</div>';
            echo '<div class="laptop-details">';
            echo '<div class="details-wrapper">';
